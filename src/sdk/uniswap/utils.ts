...
export function getPairToken(
  ...
  return {
    symbol: `${token0.symbol}/${token1.symbol} ZLP`,
    name: `ZkDex ${token0.symbol}/${token1.symbol} LP Token`,
  };
}

export function computeZkSalt(token0: Token, token1: Token) {
  return ethers.utils.keccak256(
    ethers.utils.concat([token0.address, token1.address]),
  );
}
